...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  const [cart, setCart] = useState<Product[]>(() => {
    const storagedCart = localStorage.getItem("@RocketShoes:cart"); //busca od dados do localStorage
  ...
  const addProduct = async (productId: number) => {
    try {
      ...
      const stock = await api.get(`/stock/${productId}`); //faz a requisição do estoque de produtos.

      const stockAmount = stock.data.amount; //quantidade que tem no estoque
